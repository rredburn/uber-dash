...
def update_total_rides(datePicked):
    ...
    return "Total Number of rides: {:,d}".format(len(totalList[date_picked.month - 4][date_picked.day - 1]))
...
def update_total_rides_selection(datePicked, selection):
    ...
    if datePicked is None or selection is None or len(selection) == 24 or len(selection) == 0:
